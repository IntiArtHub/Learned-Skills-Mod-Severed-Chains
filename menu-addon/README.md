...
> [!WARNING]
> This test package supports only Severed Chains `3.0.0-1779-devbuild`, commit `b2a49e4df549b1941b1c5af45d0d150f27c2dc4e`. Do **not** update Severed Chains while it is installed. Uninstall the Menu Addon **before** updating Severed Chains.
...
Install the local v1.1.0 release candidate core JAR first. In the player package, run `Verify Menu Addon.bat`, then `Install Menu Addon.bat`. The installer requests the game directory, verifies all relevant hashes, shows the mandatory warning, and requires typing `INSTALL`. This remains a private 1779 test kit and is not a published v1.1 release artifact.
...
The exact 1779 engine source is the commit recorded in the installer package and must match its SHA-256 checks.
