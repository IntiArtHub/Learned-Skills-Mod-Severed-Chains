...
/** Grants Quickchange while equipped; mastery permanently grants the action. */
public final class QuickchangeManualEquipment extends Equipment {
  public QuickchangeManualEquipment() {
  ...
  @Override public Renderable58 renderIcon(final int x, final int y, final int flags) {
    return ManualAtlasIcon.QUICKCHANGE.renderInventory(this, x, y, flags);
  }

  @Override public Renderable58 renderIconManual(final int x, final int y, final int flags) {
    return ManualAtlasIcon.QUICKCHANGE.renderInventoryManual(this, x, y, flags);
  }

  @Override public CanEquip canEquip(final CharacterData2c character, final EquipmentSlot slot) {
    return SkillSystemRuntime.hasPermanentSkill(character, SkillSystemRuntime.QUICKCHANGE) ? CanEquip.DENY : CanEquip.FORCE;
  }
}
